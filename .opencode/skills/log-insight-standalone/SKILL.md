...
| Aspect | Plugin (`split_log_chunks` tool) | Standalone (JS script) |
|--------|----------------------------------|------------------------|
| Split logic | Native opencode tool | Node.js script via Bash (.cjs) |
| Dependencies | opencode + plugin | Node.js only |
| Platform | opencode only | Any agent with Bash + Node |
| Briefing delivery | Tool argument | File (`--briefing-file`) |
| Chunk sizing | Same (3.5 bytes/token, 70%) | Same |
| Sanitization | Same (ANSI + C0) | Same |
| Agent prompt | Generated by tool | Generated by script |
| Sub-agent flow | Same (Bash cat → reason) | Same |
