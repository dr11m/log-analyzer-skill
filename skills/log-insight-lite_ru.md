---
name: log-insight-lite
description: Облегчённый индуктивный анализ логов без привязки к проекту. Работает исключительно на основе содержимого лог-файла — без брифинга, без чтения документации и исходного кода. Плагин не требуется — используется встроенный Node.js-скрипт для разделения. Работает на любой агентной платформе с Bash + Node.js. Принимает запросы вида «/log-insight-lite --chunks 5 --log logs/app.log».
---
...
**Ключевое отличие от log-insight-standalone:** этот навык работает **без контекста проекта**. Нет фазы чтения документации, нет PROJECT_BRIEFING. Суб-агенты выводят всё самостоятельно из содержимого лога: формат, компоненты, бизнес-логику, ожидаемое поведение.

Этот навык намеренно индуктивный: каждый суб-агент получает один полный чанк лога и инструкции по анализу. Суб-агенты делают ровно один вызов инструмента для получения чанка, а затем рассуждают исключительно на основе полученного содержимого.

### Бюджет контекста (токены, не байты)

Флаг `--context K` задаёт окно контекста суб-агента в **тысячах токенов** (например, 200 = 200K, 700 = 700K). Каждый чанк рассчитан на **80% этого окна** (больше, чем в log-insight-standalone, т.к. нет брифинга), оставляя место для инструкций и ответа суб-агента.
...
Примеры: `/log-insight-lite --chunks 5 --log logs/app.log`, `/log-insight-lite --chunks 3`, `/log-insight-lite --chunks 5 --context 700`.

Если `--chunks` отсутствует, спроси у пользователя. Если `--log` отсутствует, автоматическое обнаружение.
...
## Фаза 1: Разбей лог на чанки

В отличие от log-insight-standalone, **здесь нет фазы сбора контекста проекта**. Переходим сразу к разбиению.

### Шаг 1: Запиши скрипт разбиения

Запиши следующий JavaScript в `.opencode/split-log-lite.cjs` (расширение `.cjs` гарантирует совместимость с CommonJS, даже если `package.json` проекта содержит `"type": "module"`):
...
if (!logPath || !requestedChunks) {
  console.error("Usage: node split-log-lite.cjs --log <path> --chunks <N> [--context <K>] [--chunks-dir <dir>]");
  process.exit(1);
}
...
const CHUNK_BUDGET_RATIO = 0.8; // 80% — больше, чем в standalone (70%), т.к. нет брифинга
...
if (linesByContext < linesByDivision) {
  ...
  warnings.push(
    "chunk_size capped at 80% of context (" + linesPerChunk + " lines/chunk). " +
...
// --- Шаблон промпта агента (без PROJECT_BRIEFING) ---
const PROMPT_TEMPLATE = [
  ...
  "",
  "Purpose:",
  "This is context-free inductive log analysis. You have NO project documentation or briefing.",
  "You must infer everything from the log content itself:",
  "- Log format and structure (timestamp format, log levels, component names)",
  "- Application type and domain (web server, microservice, batch job, etc.)",
  "- Components and their roles (from logger names, module paths, class names in log lines)",
  "- Expected behavior patterns (from successful operations visible in the log)",
  "- Business logic hints (from variable names, operation names, data patterns)",
  "",
  "Chunk metadata:",
  ...
  "",
  "Analysis checklist:",
  "0. Log format discovery (first 20-50 lines):",
  "   - Identify timestamp format, log level format, component/module naming.",
  "   - Identify the application type (web framework, queue worker, scheduler, etc.).",
  "   - Note any structured logging (JSON lines, key=value pairs).",
  ...
  "   - Infer expected workflows from successful operation sequences and flag deviations.",
  ...
  "",
  "### Discovered Context",
  "- **Log format:** <timestamp format, level format, component format>",
  "- **Application type:** <inferred type>",
  "- **Active components:** <list of components/modules seen in this chunk>",
  "- **Observed workflows:** <brief description of normal operation patterns>",
  "",
  "### CRITICAL",
  "- **<title>**: <description, exact count, first/last timestamp>",
  "  - Evidence: `<representative log line>`",
  "  - Inferred expected behavior: <what the system was probably trying to do>",
  ...
  "- `### Discovered Context` is present.",
  "- `### Cross-Chunk Signals` is present.",
].join("\n");

function renderAgentPrompt(chunkNumber, totalChunks, byteSize, lineCount, chunkFile, offset) {
...
for (let i = 1; i <= requestedChunks; i++) {
  ...
  manifestChunks.push({
    ...
    agent_prompt: renderAgentPrompt(i, requestedChunks, byteSize, lineCount, chunkFile, offset),
  });
}
...
```
node .opencode/split-log-lite.cjs --log <абсолютный-путь> --chunks <N> --context <K>
```
...
## Фаза 2: Запусти по одному суб-агенту на чанк
...
`chunks[i].agent_prompt` компактен (~10-30 КБ, меньше чем в standalone — нет брифинга). Он содержит инструкции анализа + одну директиву Bash:
...
- НЕ подставляй плейсхолдеры — их не осталось, скрипт сгенерировал полностью готовые промпты.
...
Дождись завершения всех суб-агентов. Финальный отчёт строй только на основе ответов суб-агентов. Не открывай файл лога для собственного анализа.

### Шаг 1: Объедини обнаруженный контекст

Собери секции `### Discovered Context` из всех чанков в единое описание системы:
- Формат логов (должен быть одинаковый — если разный, отметь)
- Тип приложения (консенсус из всех чанков)
- Полный список активных компонентов (объединение из всех чанков)
- Наблюдаемые рабочие процессы (объединение)

### Шаг 2: Объедини дублирующиеся находки
...
### Шаг 3: Построй временнýю шкалу метрик

Из каждой секции `### Cross-Chunk Signals`:
...
## Отличия от других навыков

| Аспект | log-insight-standalone | log-insight-lite (этот) |
|--------|----------------------|------------------------|
| Контекст проекта | Да (Phase 1: брифинг) | Нет — только лог |
| Фаз всего | 4 (брифинг → разбиение → агенты → отчёт) | 3 (разбиение → агенты → отчёт) |
| Бюджет чанка | 70% контекста | 80% контекста (больше места под лог) |
| Скрипт | split-log.cjs | split-log-lite.cjs |
| Требует знание проекта | Да | Нет |
| Суб-агент выводит контекст | Нет (получает готовый) | Да (секция Discovered Context) |
| Применение | Логи своего проекта | Любые логи, чужие системы, быстрый анализ |
| Зависимости | Node.js + документация проекта | Только Node.js |
