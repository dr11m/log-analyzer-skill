...
4. Principle **"split — pass through"**: for each chunk `i`, take `chunks[i].agent_prompt` and pass it to the **Task** tool as ONE STRING, VERBATIM. Between `split_log_chunks` and `Task` there must be ZERO other tool calls — no `Bash`/`Write`/`Read`/`Edit`/`Python`, no saving to files, no segmenting, no placeholder substitutions. The full list of forbidden manipulations is in Phase 3 of the skill — read it before launching Tasks. Launch all N Task calls in a single response block so they run in parallel. Sub-agents have zero tool budget (no Read, no Grep, no Bash, no split_log_chunks).
